...
public class GoodBadTouch implements Serializable {
    ...
    private ArrayList<ReadingSession.Touch> _ReadEyeCoordinates = new ArrayList<>();
    private ArrayList<ReadingSession.Touch> _PostReadEyeCoordinates = new ArrayList<>();
    ...
    public void checkEyeDuringReadValidity(int page, ArrayList<View> allViews, float x, float y) {
        Calendar calendar = Calendar.getInstance();
        for ( View v : allViews ) {
            int[] location = new int[2];
            if ( v != null ) {
                ...
                if ( x >= vx
                    ...
                    _ReadEyeCoordinates.add(new ReadingSession.Touch(calendar, x, y, true));
                    _saveCSVEye.saveData(page, x, y, true, v.getResources().getResourceName(v.getId()));
                }
            }
        }
        _ReadEyeCoordinates.add(new ReadingSession.Touch(calendar, x, y, false));
        _saveCSVEye.saveData(page, x, y, false," ");


    }
    public void checkEyePostReadValidity(int page, ArrayList<View> allViews, float x, float y) {
        Calendar calendar = Calendar.getInstance();
        for ( View v : allViews ) {
            int[] location = new int[2];
            if ( v != null ) {
                ...
                if ( x >= vx
                    ...
                    _PostReadEyeCoordinates.add(new ReadingSession.Touch(calendar, x, y, true));
                    _saveCSVEye.saveData(page, x, y, true, v.getResources().getResourceName(v.getId()));
                }
            }
        }
        _PostReadEyeCoordinates.add(new ReadingSession.Touch(calendar, x, y, false));
        _saveCSVEye.saveData(page, x, y, false," ");
...
*/
    ...
    public ArrayList<ReadingSession.Touch> get_PostReadEyeCoordinates() {
        return _PostReadEyeCoordinates;
    }

    public ArrayList<ReadingSession.Touch> get_ReadEyeCoordinates() {
        return _ReadEyeCoordinates;
    }
